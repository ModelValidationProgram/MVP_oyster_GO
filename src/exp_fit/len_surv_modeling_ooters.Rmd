...
lmm5 <- lm(surv ~ bag_site * pop, data = surv_mod_ord_rm[surv_mod_ord_rm$t == 4,])
plot(lmm5)
...
# now redo model
lmm5.1 <- lm(surv ~ bag_site * oldpop, data = surv_t4)
plot(lmm5.1)
...
tuk_surv_t4 <- TukeyHSD(aov(lmm5.1), conf.level = 0.95)
cld_surv_t4 <- multcompLetters4(aov(lmm5.1), tuk_surv_t4)
...
sink(paste0("results/survival_model_results_05292025", Sys.Date(), ".txt"))
Anova(lmm4.1, type = "III")
Anova(lmm5, type = "III")
...
ggsave("results/figures/survival_t4_h2f.pdf", plot = surv_t4_plot_pop, width = 10, height = 6, dpi = 300)
```
...
# one more model on final time point only
lmm10 <- lmer(length ~ bag_site * pop + (1|bags_label), data = lengths_mod_narm_nodead[lengths_mod_narm_nodead$t == 4,])
plot(lmm10)
...
# york t4
surv_GE4.york <- lm(surv ~ ho + ar + max_temp + min_temp + max_sal + min_sal, data = surv_mod_gen_env[surv_mod_gen_env$t == 4 & surv_mod_gen_env$bag_site == "YorkRiver",])
plot(surv_GE4.york)
